...
def _get_ssh() -> paramiko.SSHClient:
    ...
    if _SSH_CLIENT is None:
        ...
        c.connect("qbd.loni.org", username="jcarde7"); _SSH_CLIENT = c
    return _SSH_CLIENT
def _ssh_exec(cmd: str) -> str:
    _, s, _ = _get_ssh().exec_command(cmd); return s.read().decode().strip()
...
def create_generation(gen: int) -> Path:
    ...
    for i in range(1, POP + 1):
        cdir = gdir / f"child_{i:02d}"; cdir.mkdir(exist_ok=True)
...
def submit_and_wait():
    ...
    while _ssh_exec(f"squeue -h -j {jid}"): time.sleep(300)

def avg_rg(child: Path) -> float:
    u = mda.Universe(child/"polymer_drug_solvate_ion.psf", child/"run_pr.dcd")
    sel = u.select_atoms("not (name W HOH Q1A Q2A TQ5)")
    vals=[]
    for _ in u.trajectory[-100:]:
        com = sel.center_of_mass()
        vals.append(np.sqrt(((sel.positions-com)**2).sum(1).dot(sel.masses)/sel.masses.sum()))
    return float(np.mean(vals))

# -------- probability utils --------
ProbDict = Dict[str,np.ndarray]
def uniform_prob()->ProbDict: return {k:np.ones(6)/6 for k in ("C","N","Qplus","Qminus")}
def orthogonal_seed()->List[Tuple[int,int,int,int]]:
    b=list(range(6)); design=[(i,b[(i+2)%6],b[(i+4)%6],b[(i+1)%6]) for i in b]
    random.shuffle(design); return design[:POP]

def draw_batch(prob:ProbDict,best:Tuple[int,int,int,int])->List[Tuple[int,int,int,int]]:
    out=[best]
    for _ in range(EXPLOIT_K-1):
        c,n,qp,qm=best; pos=random.randint(0,3)
        if pos==0: c=np.random.choice(6,p=prob["C"])
        elif pos==1: n=np.random.choice(6,p=prob["N"])
        elif pos==2: qp=np.random.choice(6,p=prob["Qplus"])
        else: qm=np.random.choice(6,p=prob["Qminus"])
        out.append((c,n,qp,qm))
    while len(out)<POP:
        out.append((np.random.choice(6,p=prob["C"]),
                    np.random.choice(6,p=prob["N"]),
                    np.random.choice(6,p=prob["Qplus"]),
                    np.random.choice(6,p=prob["Qminus"])))
    return out

def update_prob(prob:ProbDict, recs:List[Tuple[Tuple[int,int,int,int],float]])->ProbDict:
    recs.sort(key=lambda x:x[1]); cut=max(1,int(len(recs)*TOP_FRAC))
    delta={k:np.zeros(6) for k in prob}
    for (c,n,qp,qm),_ in recs[:cut]:
        delta["C"][c]+=1; delta["N"][n]+=1; delta["Qplus"][qp]+=1; delta["Qminus"][qm]+=1
    for (c,n,qp,qm),_ in recs[cut:]:
        delta["C"][c]-=1; delta["N"][n]-=1; delta["Qplus"][qp]-=1; delta["Qminus"][qm]-=1
    for k in prob:
        p=prob[k]+DAMP*delta[k]; p[p<0]=0.001; prob[k]=p/p.sum()
        if random.random()<RESEED_CHANCE: prob[k]=(prob[k]+1/6)/(1+1)
    return prob

# -------- logging --------
def log_csv(path:Path,gen:int,child:int,combo,rg,err):
    hdr=not path.exists()
    with path.open("a",newline="") as f:
        w=csv.writer(f); 
        if hdr: w.writerow(["gen","child","C","N","Qplus","Qminus","Rg","err"])
        c,n,qp,qm=combo
        w.writerow([gen,child,C_LEVELS[c],N_LEVELS[n],Q_LEVELS[qp],Q_LEVELS[qm],
                    f"{rg:.3f}",f"{err:.3f}"])

# -------- main loop --------
def in_window(r:float)->bool: return LOWER_RG<=r<=UPPER_RG

def main():
    prob=uniform_prob(); csv_file=REPO/"optimisation_results.csv"
    total_runs=0; best=None
    for gen in itertools.count(0):
        batch = orthogonal_seed() if gen==0 else draw_batch(prob,best["combo"])
        gdir=create_generation(gen)
        for i,combo in enumerate(batch,1): write_psf(gdir/f"child_{i:02d}",combo)
        write_slurm(gen); submit_and_wait()

        recs=[]
        for i,combo in enumerate(batch,1):
            rg=avg_rg(gdir/f"child_{i:02d}"); err=abs(rg-TARGET_RG)
            recs.append((combo,err)); log_csv(csv_file,gen,i,combo,rg,err)
        ...
        if (best is None) or (err_best < best["err"]):
            best={"combo":combo_best,"Rg":TARGET_RG-err_best if rg<TARGET_RG else TARGET_RG+err_best,"err":err_best}

        total_runs+=POP
        if in_window(best["Rg"]): break
        if total_runs>=TOTAL_CAP or gen>=GEN_MAX: break
        prob=update_prob(prob,recs)

    c,n,qp,qm=best["combo"]
    print(f"\nBest: C={C_LEVELS[c]} N={N_LEVELS[n]} Q+={Q_LEVELS[qp]} Q-={Q_LEVELS[qm]}  "
          f"Rg={best['Rg']:.3f} Å |Δ|={best['err']:.3f} Å")
    if _SSH_CLIENT: _SSH_CLIENT.close()
